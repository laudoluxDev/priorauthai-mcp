...
Tools (names are vertical-prefixed at runtime, e.g. priorauthai_search):
...
API_BASE    = os.getenv("TASKSAI_API_BASE", os.getenv("LAWTASKSAI_API_BASE", "https://api.priorauthai.com"))
...
async def api_get(path):
    async with httpx.AsyncClient(timeout=30.0) as client:
        resp = await client.get(
            f"{API_BASE}{path}",
            headers={**AUTH_HEADERS, "X-Product-ID": (_vertical or {}).get("product_id", "priorauthai")}
        )
...
async def load_vertical():
    """Fetch vertical metadata from /v1/me on startup. Falls back to PriorAuthAI."""
    global _vertical
    try:
        ...
        prefix = LICENSE_KEY.split("_")[0] + "_" if "_" in LICENSE_KEY else "pa_"
        _vertical = {
            "product_id":   "priorauthai",
            "product_name": "PriorAuthAI",
            "display_name": "PriorAuthAI",
            "tool_prefix":  "priorauthai",
            "occupation":   "healthcare prior authorization teams",
            "support_email":"support@priorauthai.com",
            "domain":       "priorauthai.com",
        }
    return _vertical
    ...
    return [
        Tool(
            ...
            description=(
                ...
                f"domain-specific checklists, review steps, and document generation templates "
...
def normalize_audience_label(label):
    ...
    if label in {"attorney", "lawyer"}:
        return f"{label}s"
    if label in {"prior authorization", "healthcare"}:
        return "healthcare prior authorization teams"
    return label
...
def normalize_domain_adjective(label):
    ...
    if label in {"prior authorization", "healthcare", "healthcare prior authorization teams"}:
        return "prior authorization"
    if label in {"attorney", "lawyer", "law"}:
        return "legal"
    return label
...
# Phase 1: create server with placeholder tools (PriorAuthAI defaults)
...
_system_prompt_text = build_system_prompt(
    "PriorAuthAI", "healthcare prior authorization teams",
    "priorauthai", "priorauthai.com", "support@priorauthai.com"
)
...
server = Server("priorauthai")

# Placeholder tools using PriorAuthAI defaults (overwritten after /v1/me loads)
_tools = build_tools("priorauthai", "PriorAuthAI", "healthcare prior authorization teams")
...
def _rebuild_tools():
    ...
    if _vertical is None:
        return
    prefix   = _vertical.get("tool_prefix", "priorauthai")
    name     = _vertical.get("product_name", "PriorAuthAI")
    occ      = _vertical.get("occupation", "professionals")
    domain   = _vertical.get("domain", "priorauthai.com")
    support  = _vertical.get("support_email", "support@priorauthai.com")
...
async def call_tool(name, arguments):
    ...
    v          = _vertical or {}
    prefix     = v.get("tool_prefix", "priorauthai")
    product_id = v.get("product_id", "priorauthai")
    product_name = v.get("product_name", "PriorAuthAI")
    ...
    try:
        ...
        # ── Balance ───────────────────────────────────────────────────────────
            ...
            domain   = v.get("domain", "priorauthai.com")
            return [TextContent(type="text", text=(
